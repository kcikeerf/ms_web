...
class Analyzer < ActiveRecord::Base
  ...
  class << self
    def get_list params
      ...
      conditions = []
      conditions << self.send(:sanitize_sql, ["analyzers.name LIKE ?", "%#{params[:name]}%"]) unless params[:name].blank?
      conditions << self.send(:sanitize_sql, ["analyzers.subject LIKE ?", "%#{params[:subject]}%"]) unless params[:subject].blank?
      ...
      result = self.joins(:user, :tenant).where(conditions).order("dt_update desc").page(params[:page]).per(params[:rows])
