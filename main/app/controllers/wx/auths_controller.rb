class Wx::AuthsController < ApplicationController
  ...
  def wx_bind
    ...
    unless params[:school_number].blank? && params[:stu_number].blank? && params[:wx_token].blank?
      ...
      if target_user && target_pupil
      	if target_user.update(:wx_token => params[:wx_token].strip)
      ...
      result = response_json(400, {message:I18n.t("wx_users.messages.warn.invalid_params")}) 
    end
    render :json => result
  end
end
