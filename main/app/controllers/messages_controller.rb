class MessagesController < ApplicationController
	...
	def send_email_auth_number
		email = params[:email]
		send_email(email) do 
			UserMailer.send_auth_number(email).deliver_now
		end
	end
	...
	def send_email_forgot_password
		email = params[:email]
		send_email(email) do 
			UserMailer.forgot_password(email).deliver_now
		end
	end
...
private
	...
	def send_email(email)
		...
		return render json: response_json(500, I18n.t('messages.email_not_send')) if $cache_redis.get(email_count_key)

		yield
		$cache_redis.incr(email_count_key)
		$cache_redis.expire(email_count_key, 1.minute)
	...
	def send_sms(mobile, content, type, auth_number)
		key = "#{type}_#{mobile}"
		return render json: response_json(500, I18n.t('messages.send_limit')) if $cache_redis.get(key)
		...
		$cache_redis.expire(key, 1.minute)
