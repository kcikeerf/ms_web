...
class ImportResultsJob < ActiveJob::Base
    ...
    #Common::process_sync_template(__method__.to_s()) {|pids|
      # pids << fork do # fork new process, begin
        begin
          ...
          # temp = @target_test.bank_test_tenant_links.where(:tenant_uid => params[:tenant_uid]).first
          # temp.update({
  ...
  def import_score_core args={}
      ...
      #p "线程（#{@target_tenant}）：>>>thread index #{args[:th_index]}: [from, to]=>[#{args[:start_num]},#{args[:end_num]}] <<<"
      begin
        #主线程读取用户信息
        teacher_username_in_sheet = []
        pupil_username_in_sheet = []
        # qzps_h = []
        # @hidden_row[args[:data_start_col]..(@total_cols-1)].each{|item|
        ...
        (args[:start_num]..args[:end_num]).each{|index|
          ...
          loc_key = @target_tenant.uid + cells[:grade] + cells[:classroom]
          ...
          # p "线程（#{@target_tenant}）：>>>thread index #{args[:th_index]}: row:#{index},  cols range: #{args[:data_start_col]},#{@total_cols} <<<"
          ...
          # p "线程（#{@target_tenant}）：>>>thread index #{args[:th_index]}: redis count=>#{Common::SwtkRedis::get_value(@redis_key)}"
          ...
          #p "线程（#{@target_tenant}）：>>>thread index #{args[:th_index]}: process_value => #{process_value}"
